...
GA::GA(int num,int chosen,int original_length,int chance,std::vector<void(*)(const float & ,const float & ,float & )> avops,std::vector<std::string> avops_symb,int ran_max,int ran_min,float k_a,float k_b,std::vector<std::pair<float,float>> train_data){
    GA::avops = avops;
    GA::num = num; 
    GA::chosen = chosen;
    GA::chance = chance;
...
void GA::sort(){
	...
	if (std::isnan(a.second) || std::isinf(a.second)){return false;}	
