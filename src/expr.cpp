...
void Expression::generate_random(int n)
{
    if (n < 1)
    {
        throw std::invalid_argument("original length >= 1");
    }
    delete_tree(Expression::tree);
    // generate nodes
...
{ // chance <- percentage, mutate 1 time
    if (rand() % 100 < chance)
    {
        //mutate leaves
        Node **p = NULL;
        ...
        { //modify
            ...
            if (p == NULL)
            {
                throw std::runtime_error("Unknown error in modify stage");
            }
...
void Expression::mutate_keep_length(int chance)
{
    if (rand() % 100 < chance)
    {
        Node **p = NULL;
        ...
        if (p == NULL)
        {
            throw std::runtime_error("Unknown error in delete stage");
        }
        Node *temp = create_node();
        fill_tree(temp);
        temp->left = (*p)->left;
